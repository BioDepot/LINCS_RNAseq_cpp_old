...
DPREFIX="/local/";
DOCKER_CPP="docker run --rm -w /local -e NWELLS=${NWELLS} -v ${PWD}:/local biodepot/rnaseq-umi-cpp:3.7-1.0 "
DOCKER_BWA="docker run --rm -w /local -v ${PWD}:/local biodepot/alpine-multibwa:3.7-0.7.15 "
...
echo "$DOCKER_BWA /bin/multibwa.sh $TOP_DIR $REF_DIR $SPECIES_DIR $ALIGN_DIR $BWA_ALN_SEED_LENGTH $BWA_SAM_MAX_ALIGNS_FOR_XA_TAG $THREAD_NUMBER"
$DOCKER_BWA /bin/multibwa.sh  $TOP_DIR $REF_DIR $SPECIES_DIR $ALIGN_DIR $BWA_ALN_SEED_LENGTH $BWA_SAM_MAX_ALIGNS_FOR_XA_TAG $THREAD_NUMBER 
